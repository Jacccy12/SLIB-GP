...
sys.path.append('../BiDOhsiccoco')
import model
#训练一个生成器（Generator, G）和一个判别器（Discriminator, DG）
...
if __name__ == "__main__":
    ...
    parser.add_argument('--defense', default='reg', help='reg | vib | vib_dp')
    ...
    if args.dataset == 'celeba':
        ...
        if args.defense == 'vib_dp':
            ...
            path_T = os.path.join("../BiDO/target_model/{}".format(args.dataset),
                                  f"{model_name}_progressive_dp_{ac:.2f}.tar")
        ...
        if args.defense == 'vib':
            ...
            beta = args.beta = 0.005
            ac = args.acc = 99.82

            T = model.MCNN_vib(n_classes)
            T = torch.nn.DataParallel(T).cuda()
            # path_T = os.path.join("../BiDOhsiccoco/target_model/{}".format(args.dataset), args.defense,
            #                       f"{model_name}_beta{beta:.3f}_{ac:.2f}.tar")
            path_T = os.path.join("../BiDOhsiccoco/target_model/{}".format(args.dataset),
                                  f"{model_name}_beta{beta:.3f}_{ac:.2f}.tar")
            ckp_T = torch.load(path_T)
            T.load_state_dict(ckp_T['state_dict'])
        ...
        if args.defense == 'vib':
            ...
            # beta, ac = 0.01, 87.46  # 可以根据需要调整

            # T = model.VGG16_vib(n_classes)
            T = model.VGG16(n_classes, dataset='cifar')
            T = torch.nn.DataParallel(T).cuda()
            path_T = os.path.join("../BiDOhsiccoco/target_model/{}".format(args.dataset),
                                  f"{model_name}_beta{beta:.3f}_{ac:.2f}.tar")
            ckp_T = torch.load(path_T)
            ...
            Gpath = os.path.join(save_model_dir, "{}_G_beta_{:.3f}_{:.2f}.tar").format(model_name, beta, ac)
            Dpath = os.path.join(save_model_dir, "{}_D_beta_{:.3f}_{:.2f}.tar").format(model_name, beta, ac)
        elif args.defense == 'vib_dp':
            beta, ac = 0.01, 79.95  # 可以根据需要调整
            T = model.VGG16_vib(n_classes)
            T = torch.nn.DataParallel(T).cuda()
            path_T = os.path.join("../BiDO/target_model/{}".format(args.dataset),
                                  f"{model_name}_progressive_dp_{ac:.2f}.tar")
    ...
    if args.dataset == 'celeba':
        ...
        DG = MinibatchDiscriminator_CIFAR(n_classes=10)
