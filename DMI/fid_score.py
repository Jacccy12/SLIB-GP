...
# import sys
# sys.path.append('/home/jys/.virtualenvs/Defend_MI-master/BiDO/')
#
...
# 获取 BiDO 目录的绝对路径（假设 BiDO 和 DMI 是同级目录）
bido_dir = os.path.join(current_dir, '../BiDO')
# 将 BiDO 目录添加到 Python 路径
sys.path.append(bido_dir)

# 现在可以正确导入 BiDO 下的 model 模块
from model import SCNN, VGG19
import evolve, utils
...
def calculate_fid_given_paths(dataset, paths, batch_size=50, cuda=1, dims=2048):
    ...
    if dataset == 'celeba':
        ...
        model = FaceNet(1000)
        model = torch.nn.DataParallel(model).cuda()
        path_E = '/home/jys/.virtualenvs/Defend_MI-master/DMI/eval_ckp/FaceNet_95.88.tar'
        ckp_E = torch.load(path_E)
        model.load_state_dict(ckp_E['state_dict'], strict=False)
