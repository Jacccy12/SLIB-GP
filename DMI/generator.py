...
#         self.z_dim = z_dim
#
#         # 假设原始 Generator 是从 z -> 4x4 -> 8x8 -> 16x16 -> 32x32 -> 64x64
#         # 我们需要让它停在 32x32
#         self.fc = nn.Linear(z_dim, 512 * 2 * 2)  # 基础特征图 2x2
#
#         def deconv_bn_relu(in_dim, out_dim, k, s, p):
#             return nn.Sequential(
#                 nn.ConvTranspose2d(in_dim, out_dim, k, s, p),
#                 nn.BatchNorm2d(out_dim),
#                 nn.ReLU(True))
#
#         self.layer1 = deconv_bn_relu(512, 256, 5, 2, 2)  # 2x2 -> 4x4
#         self.layer2 = deconv_bn_relu(256, 128, 5, 2, 2)  # 4x4 -> 8x8
#         self.layer3 = deconv_bn_relu(128, 64, 5, 2, 2)  # 8x8 -> 16x16
#         self.layer4 = nn.Sequential(
#             nn.ConvTranspose2d(64, 3, 5, 2, 2),  # 16x16 -> 32x32
#             nn.Tanh())
#
#     def forward(self, z):
#         x = self.fc(z)
...
class Generator_CIFAR(nn.Module):
    def __init__(self, z_dim):
        ...
        # 改进的DCGAN架构：从4x4开始，更稳定的结构
        # 使用更大的初始特征图尺寸，有助于生成更清晰的图像
        ...
        def deconv_bn_relu(in_dim, out_dim, k, s, p, op=0):
            layers = [
                nn.ConvTranspose2d(in_dim, out_dim, k, s, p, output_padding=op, bias=False),
                nn.BatchNorm2d(out_dim),
                nn.ReLU(True)
            ]
        ...
        # 4x4 -> 8x8 -> 16x16 -> 32x32
        self.layer1 = deconv_bn_relu(512, 256, 4, 2, 1)  # 4x4 -> 8x8
        self.layer2 = deconv_bn_relu(256, 128, 4, 2, 1)  # 8x8 -> 16x16
        self.layer3 = deconv_bn_relu(128, 64, 4, 2, 1)  # 16x16 -> 32x32

        # 输出层：使用Tanh确保输出在[-1, 1]范围
        self.layer4 = nn.Sequential(
            nn.ConvTranspose2d(64, 3, 3, 1, 1, bias=False),  # 保持32x32
            nn.Tanh()
        )
