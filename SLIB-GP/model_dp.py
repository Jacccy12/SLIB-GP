...
#综合性的深度学习模型库
class VGG19(nn.Module):#经典CNN，支持HSIC训练和变分信息瓶颈
...
class Flatten(nn.Module):#将多维特征张量展平为一维（用于全连接层输入）
    def forward(self, input):
        return input.view(input.size(0), -1)

class CrossEntropyLoss(_Loss):#自定义交叉熵损失，支持动态平滑标签
...
class CrossEntropyLossMaybeSmooth(nn.CrossEntropyLoss):#上一个优化
    ...
    def forward(self, output, target, smooth=False):
        if not smooth:
            return F.cross_entropy(output, target)#不平滑直接使用 PyTorch 默认的交叉熵损失计算方法

        target = target.contiguous().view(-1)#平展为一维向量
        n_class = output.size(1)#类别数
        one_hot = torch.zeros_like(output).scatter(1, target.view(-1, 1), 1)#变成onehot
        smooth_one_hot = one_hot * (1 - self.smooth_eps) + (1 - one_hot) * self.smooth_eps / (n_class - 1)#标签平滑
        log_prb = F.log_softmax(output, dim=1)
        loss = -(smooth_one_hot * log_prb).sum(dim=1).mean()
        return loss


class BinaryLoss(_Loss):#二分类交叉熵损失
...
####################################### MNIST ###########################################
def get_in_channels(data_code):#根据数据集名称返回输入图像的通道数
    in_ch = -1
    if data_code == 'mnist':
        in_ch = 1
...
class MCNN(nn.Module):#3个卷积层一个全连接层
    def __init__(self, num_classes=5):
        ...
        self.layer3 = nn.Sequential(nn.Conv2d(128, 256, 3, stride=1, padding=1),  # Changed from 5x5 to 3x3 with padding
                                    nn.BatchNorm2d(256),
                                    nn.LeakyReLU(0.2))

        # Use adaptive average pooling to ensure consistent output size
        self.adaptive_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc_layer = nn.Linear(256, self.num_classes)
...
class MCNN_vib(nn.Module):#带有变分信息瓶颈
    def __init__(self, num_classes=5):
        super(MCNN_vib, self).__init__()
        self.feat_dim = 256#批次
        self.num_classes = num_classes
        self.layer1 = nn.Sequential(
            nn.Conv2d(1, 64, 7, stride=1, padding=1),
            nn.BatchNorm2d(64),
...
#全连接层
        ...
        # 全连接层，接受展平后的特征（256维），输出 k * 2 维（即256），用于生成潜在变量的均值 (mu) 和标准差 (std) 参数。
        self.fc_layer = nn.Linear(self.k, self.num_classes)#分类层
    ...
    def forward(self, x):
        ...
        statis = self.st_layer(feature)# [N,256] → [N,256]
        mu, std = statis[:, :self.k], statis[:, self.k:]


        std = F.softplus(std - 5, beta=1) # 确保标准差>0
        eps = torch.FloatTensor(std.size()).normal_().cuda()
        # 生成一个与标准差 std 形状相同的随机噪声张量 eps，它来自标准正态分布。
        res = mu + std * eps# 得到潜在变量z[N,128]
        out = self.fc_layer(res)

        return [feature, mu, std, out]#生成潜在变量。
...
############################################ VGG ###################################################
def make_layers(cfg, batch_norm=False):#根据给定的配置创建卷积神经网络层的函数
    blocks = []
    layers = []
    in_channels = 3
    for v in cfg:
...
class VGG16(nn.Module):
    def __init__(self, n_classes, hsic_training=False, dp_training=False, dataset='celeba'):
        ...
        self.hsic_training = hsic_training#布尔值用于决定网络是否使用“HSIC training”
        ...
        if self.hsic_training:
            ...
            model = torchvision.models.vgg16_bn(pretrained=True)#将使用 torchvision 中的预训练 VGG16 网络
            self.feature = model.features

        if dataset == 'celeba':#数据集
            self.feat_dim = 512 * 2 * 2
        else:
            self.feat_dim = 512
        self.n_classes = n_classes
        self.bn = nn.BatchNorm1d(self.feat_dim)#批量归一化层
        if not dp_training:
            self.bn.bias.requires_grad_(False)  # no shift禁用了偏置项的梯度更新
        self.fc_layer = nn.Linear(self.feat_dim, self.n_classes)#全连接层
...
class VGG16_LS(nn.Module):
    def __init__(self, n_classes, hsic_training=False, dp_training=False, dataset='celeba'):
        super(VGG16_LS, self).__init__()
        ...
        if self.hsic_training:
            ...
            self.feature = model.features
            # 只加载features部分权重，不加载classifier权重
            # fc_layer始终用自定义的nn.Linear
...
class BasicBlock(nn.Module):#残差块引入了跳跃连接（skip connection）
    expansion = 1
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, num_blocks, stride):#残差块类型,输出的通道数。在当前层中创建的残差块数量。
    ...
    def forward(self, x):
        ...
        out = F.relu(self.bn1(self.conv1(x)))
        output_list.append(out)#用来保存网络中间层的输出，以便后续使用或调试。
...
class IR152_vib(nn.Module):#基于卷积神经网络的分类模型
    def __init__(self, num_classes=1000):
...
class ResNetClsH(nn.Module):#图像分类问题
    def __init__(self, nc=3, zdim=2, imagesize=32, nclass=10, resnetl=10, dropout=0):
        super(ResNetClsH, self).__init__()
        self.backbone = ResNetL_IH(resnetl, imagesize, nc)#特征提取
        self.fc1 = nn.Linear(self.backbone.final_feat_dim, zdim)#全连接层
        self.bn1 = nn.BatchNorm1d(self.backbone.final_feat_dim)#批量归一化层
        self.fc2 = nn.Linear(zdim, nclass)
        self.bn2 = nn.BatchNorm1d(zdim)
        if dropout > 0: #Dropout 层
            self.dropout1 = nn.Dropout(dropout)
            self.dropout2 = nn.Dropout(dropout)
            self.dropout = dropout

    def embed_img(self, x, release=True):#提取嵌入图像特征
    ...
    def embed(self, x):#返回图像的嵌入表示
        return self.embed_img(x, release=False)

    def z_to_logits(self, z):#将潜在空间的表示 z 转换为分类的 logits
        z = F.relu(z)
        if 'dropout' in dir(self) and self.dropout > 0:
            z = self.dropout2(z)
        feat = self.bn2(z)
    ...
    def z_to_lsm(self, z):
        ...
        return F.log_softmax(z, dim=1), feat#转换为分类的对数概率
...
class PretrainedResNet(nn.Module):
    ...
    def z_to_logits(self, z):
        z = self.fc(z)#将提取的特征向量 z 通过全连接层 self.fc 转换为最终的类别预测 logits
        return z#预测
...
class FaceNet(nn.Module):
    def __init__(self, num_classes=1000):
        ...
        self.feat_dim = 512
        self.num_classes = num_classes
        self.fc_layer = nn.Linear(self.feat_dim, self.num_classes)#全连接
