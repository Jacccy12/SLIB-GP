...
from model import SCNN, VGG19, VGG16
import evolve, utils
...
def imread(dataset, filename):
    """
    Loads an image file into a (height, width, C) uint8 ndarray.
    NOTE:
    - For 'celeba' and 'cifar', we keep the original resolution (RGB, C=3).
    - For 'mnist', we explicitly resize to 32x32 (grayscale, C=1) so that it
      matches the input resolution used to train the SCNN evaluation model.
      Otherwise, using raw 28x28 images will cause the last 5x5 conv layer in
      SCNN to see a 4x4 feature map and crash with
      "Kernel size can't be greater than actual input size".
    """
    if dataset in ('celeba', 'cifar'):
        # RGB images, keep original size
        return np.asarray(Image.open(filename), dtype=np.uint8)[..., :3]
    elif dataset == 'mnist':
        # MNIST 原图是 28x28，这里统一 resize 到 32x32，避免 SCNN 最后 5x5 卷积报错
        img = Image.open(filename).convert('L')
        img = img.resize((32, 32), Image.BILINEAR)
        arr = np.asarray(img, dtype=np.uint8)[..., None]  # (H, W, 1)
        return arr
    elif dataset == 'cxr':
        tmp = np.asarray(Image.open(filename), dtype=np.uint8)[..., :1]
        return tmp
...
def calculate_fid_given_paths(dataset, paths, batch_size, cuda, dims):
    ...
    if dataset == 'celeba':
        ...
        path_E = '/home/jys/.virtualenvs/Defend_MI-master/GMI/eval_model/FaceNet_95.88.tar'
        ...
        path_E = '/data2/jys/.virtualenvs/Defend_MI-master/GMI/eval_model/SCNN_99.90.tar'
        ckp_E = torch.load(path_E)
        model = torch.nn.DataParallel(model).to('cuda')
        model.load_state_dict(ckp_E['state_dict'])
    elif dataset == 'cifar':
        path_E = '/data2/jys/.virtualenvs/Defend_MI-master/GMI/eval_model/VGG16_91.56.tar'
        if os.path.exists(path_E):
            model = VGG16(10, dataset='cifar')
            ckp_E = torch.load(path_E)
            model = torch.nn.DataParallel(model).to('cuda')
            model.load_state_dict(ckp_E['state_dict'], strict=False)
            dims = 512
