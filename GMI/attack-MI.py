...
# 评估期防御封装类：在推理时添加高斯噪声，模拟训练时的DP噪声效果
# 这对于MI防御模型非常重要，因为训练时使用了DP噪声，攻击时也需要添加噪声才能正确评估
# Attack should see the same stochastic interface as training
class EvalDefenseWrapper(nn.Module):
    def __init__(self, net, noise_std=0.03):
        super().__init__()
        self.net = net
        # Allow noise_std up to 0.2 to support larger DP noise values
        # Common dp_noise_std values: 0.0005, 0.001, 0.002, 0.005, 0.01, 0.02
        self.noise_std = float(max(0.0, min(noise_std, 0.2)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--defense', default='MI', help='reg | vib | HSIC|MI')
    ...
    parser.add_argument('--dp_noise_std', type=float, default=0.001,
                        help='DP noise std used during training (for MI+DP defense). Should match training dp_noise_std. Default: 0.001')
    parser.add_argument('--use_eval_noise', action='store_true',
                        help='Add evaluation-time noise for both MI and MI+DP (analysis experiment). Default: False (main experiment without noise)')
    parser.add_argument('--eval_noise_std', type=float, default=0.03,
                        help='Noise std for evaluation-time noise (only used when --use_eval_noise is True). Fixed for both MI and MI+DP. Default: 0.03')
    ...
    eval_path = "/data2/jys/.virtualenvs/Defend_MI-master/GMI/eval_model"
    ############################# mkdirs ##############################
    ...
    if args.dataset == 'celeba':
        ...
        # 使用绝对路径，防止找不到文件
        e_path = "/data2/jys/.virtualenvs/Defend_MI-master/GMI/eval_model/FaceNet_95.88.tar"
        ...
        if args.defense == 'HSIC' or args.defense == 'COCO':
            ...
            if args.defense == 'MI' or args.defense == 'MI+DP':
                hp_ac_list = [
                    # 最终的实验数据
                    # # 有噪声三组数据MI+DP
                ...
                for (a1, a2, ac) in hp_ac_list:
                    ...
                    # Load base model
                    base_T = model.VGG16(num_classes, True)
                    model_tar = f"{model_name}_{a1:.3f}&{a2:.3f}_{ac:.2f}.tar"
                    path_T = os.path.join(args.model_path, args.dataset, args.defense, model_tar)
                    ckp_T = torch.load(path_T)
                    base_T.load_state_dict(ckp_T['state_dict'], strict=False)
                    base_T = nn.DataParallel(base_T).cuda()

                    # Evaluation-time noise handling (consistent for both MI and MI+DP)
                    # Case 1: Main experiment (no eval noise) - both MI and MI+DP use base model
                    # Case 2: Analysis experiment (with eval noise) - both MI and MI+DP wrapped identically
                    if args.use_eval_noise:
                        # Analysis experiment: both MI and MI+DP get the same noise wrapper
                        T = EvalDefenseWrapper(base_T, noise_std=args.eval_noise_std)
                        T = T.cuda()
                        print(f"🔬 分析实验: MI和MI+DP统一启用评估期噪声, noise_std={args.eval_noise_std:.5f}")
                    else:
                        # Main experiment: both MI and MI+DP use base model without noise
                        T = base_T
                        print(f"📊 主实验: MI和MI+DP均不加评估期噪声 (除非特别说明，所有攻击评估均在不加推理期噪声的情况下进行)")
